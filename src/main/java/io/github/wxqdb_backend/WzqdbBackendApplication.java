...
public class WzqdbBackendApplication {

    public static void main(String[] args) throws DocumentException {
        SpringApplication.run(WzqdbBackendApplication.class, args);
...
//            @SuppressWarnings("resource")
            ...
            StringBuilder sql = new StringBuilder(input.nextLine());
//            /*
//             * 预处理:获得语句;
//             * 处理前后空格;
//             * 变小写;
...
//            //处理分行输入的问题，就是读;号才停止;
            //sql parse
            if (sql.toString().equals("help")) {
                read_help();
                continue;
            }

            while (sql.lastIndexOf(";") != sql.length() - 1) {
                sql.append(" ").append(input.nextLine());
            }

            sql = new StringBuilder(sql.toString().trim()); // 去除字符串两侧的空白字符
            sql = new StringBuilder(sql.toString().toLowerCase());//全部转为小写
            sql = new StringBuilder(sql.toString().replaceAll("\\s+", " "));// 将sql中的连续空白字符替换为一个空格
            sql = new StringBuilder(sql.substring(0, sql.lastIndexOf(";")));// 去除SQL语句末尾的分号
            sql = new StringBuilder(sql + " ENDOFSQL");
            System.out.println("1)SQL预处理结果: " + sql);

            List<List<String>> parameter_list = new ArrayList<>();//缓存sql body信息的list

            if (sql.toString().equals("quit ENDOFSQL")) {
                return;
            } else {
                //将预处理后的SQL语句匹配SQL正则表达式，返回含有SQL的body信息的List
                try {
                    parameter_list = SingleSqlParserFactory.generateParser(sql.toString());
                } catch (Exception e) {
